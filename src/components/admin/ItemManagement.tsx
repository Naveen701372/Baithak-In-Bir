...
export default function ItemManagement({ orders, onUpdateItemStatus }: ItemManagementProps) {
  ...
  const filteredItems = aggregatedItems
    .filter(item => {
      ...
      if (item.orders.length === 0 && !exitingCards.has(item.menu_item_id)) return false

      // Only show items that have pending work (not fully completed) unless they're animating out
      if (item.pending_quantity <= 0 && !exitingCards.has(item.menu_item_id)) return false

      // Don't filter by status to prevent items from moving around - show all active items
      return true
    })
